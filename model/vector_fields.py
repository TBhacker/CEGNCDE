...
class FinalTanh_f(nn.Module):
    ...
    def forward(self, z):
        ...
        #z.shape[:-1]：64 307，前面*将[64,37]解包为2个独立参数
...
class VectorField_g_gcn(torch.nn.Module):
    #weights和bias用原来的embedding和w_pool\bias_pool生成,静态图和动态图加一起
...
class VectorField_g_former(torch.nn.Module):
    #这里实现动态图former +静态图（chebet/former）
    def __init__(self, coefficient, input_channels, hidden_channels, hidden_hidden_channels, num_hidden_layers, num_nodes, cheb_k, embed_dim,
                    g_type):
        ...
        self.num_hidden_layers = num_hidden_layers
        #self.att = []
        self.geo_mask = torch.load("/home/wjj/projects/STG/data/PEMS08/geo_mask.pth")
        self.sem_mask = torch.load("/home/wjj/projects/STG/data/PEMS08/sem_mask.pth")
    ...
    def GRU(self, z, h):
        ...
        geo_attn = torch.einsum('bno,boi->bni', geo_q, geo_k.transpose(-2, -1)) 
        #(geo_q @ geo_k.transpose(-2, -1)).softmax(dim=-1)
        ############画图
        
        #plt.savefig("test.svg", dpi=300,format="svg")

        Adj_geo = self.coefficient*geo_attn +(1-self.coefficient)*A_static# 64 170 17
        a = Adj_geo.cpu().numpy()[2,:30, :30]
        plt.matshow(a, cmap=plt.cm.Blues)#这里设置颜色为红色，也可以设置其他颜色
        ...
        plt.title("time step 2")
        plt.show()
        plt.savefig("ADJ_mask.png")

        sem_q = self.sem_q_w(h)
        sem_k = self.sem_k_w(h)
        sem_v = self.sem_v_w(z)
...
class STSelfAttention(nn.Module):
    def __init__(self, geo_num_heads=4, sem_num_heads=4, hidden_channels=64,hidden_hidden_channels=64,
        ...
        self.bias_pool = nn.Parameter(torch.FloatTensor(embed_dim, hidden_hidden_channels))
        self.geo_mask = torch.load("/home/wjj/projects/STG/data/PEMS08/geo_mask.pth")
        self.sem_mask = torch.load("/home/wjj/projects/STG/data/PEMS08/geo_mask.pth")
    def forward(self, z, h):
...
class VectorField_g(torch.nn.Module):
    ...
    def agc(self, z):
        ...
        ##初步实现由z生成邻接矩阵
        # node_num = z.shape[1]
        # supports = F.softmax(F.relu(torch.mm(z[0,:,:], z[0,:,:].transpose(0, 1))), dim=1)
