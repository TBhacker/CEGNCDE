...
import time
import nni
from lib.scheduler import CosineLRScheduler

from model.BasicTrainer_cde import Trainer
from lib.TrainInits import init_seed
from lib.dataloader import get_dataloader_cde
from lib.dataloader import get_dataloader_cde_former
...
#args.add_argument('--embed_dim', default=params['embed_dim'], type=int)
args.add_argument('--embed_dim', default=config['model']['embed_dim'], type=int)
#args.add_argument('--hid_dim', default=params['hid_dim'], type=int)
args.add_argument('--hid_dim', default=config['model']['hid_dim'], type=int)
#args.add_argument('--hid_hid_dim', default=params['hid_dim'], type=int)
args.add_argument('--hid_hid_dim', default=config['model']['hid_dim'], type=int)
...
args.add_argument('--load_model', default=False, type=eval)
#####
#args.add_argument('--coefficient', default=params['coefficient'], type=float)#动态图系数
args.add_argument('--coefficient', default=0.5, type=float)
#train
...
###
#args.add_argument('--lr_init', default=params['lr_init'], type=float)
...
#config log path
save_name = time.strftime("%m-%d-%Hh:%Mm",time.localtime())+"_"+args.comment+"_"+ args.dataset+"_"+ args.model+"_"+ args.model_type+"_"+"embed{"+str(args.embed_dim)+"}"+"hid{"+str(args.hid_dim)+"}"+"hidhid{"+str(args.hid_hid_dim)+"}"+"lyrs{"+str(args.num_layers)+"}"+"lr{"+str(args.lr_init)+"}"+"wd{"+str(args.weight_decay)+"}"
#UTC时间
path = '../runs'
...
train_loader, val_loader, test_loader, scaler, times = get_dataloader_cde_former(args,
...
if args.lr_decay:
    ...
    lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer,
                                                              ...
                                                              patience=10,
                                                              factor=0.3)
    # lr_scheduler = CosineLRScheduler(optimizer=optimizer,t_initial=args.epochs, lr_min=0.0001, decay_rate=0.1,
    #                 warmup_t=5, warmup_lr_init=1e-06)
#start training
